...
async def show_result(state: FSMContext):
    ...
    dir_foto = os.path.join(os.getcwd(), f"keyboards/modul_quiz/foto")

    try:
        logging.info(f"Checking directory: {dir_foto}")
        files = os.listdir(dir_foto)
        logging.info(f"Files in directory '{dir_foto}': {files}")

        for _f in files:
            logging.info(f"Checking file: {_f}")
            if result.lower().strip() in _f.lower().strip():
                path_file = os.path.join(dir_foto, _f)
                logging.info(f"Image found for result '{result}': {_f}")
                break
        
        if path_file:
            with open(path_file, "rb") as image_from_buffer:
                image = BufferedInputFile(image_from_buffer.read(), filename=os.path.basename(path_file))
        else:
            logging.warning(f"No image found for result '{result}'")
            text += "\nК сожалению, изображение не найдено."

    except Exception as e:
        logging.error(f"Error while searching for images: {e}")
